...
for r in level:
        for c in r:
            ...
            if c == '-':
                ...
                items.add(platform)
            if c=='s':
                hero = Player('images/sprite.png', x, y,40,40, 4)
                items.add(hero)
            if c=='c':
                monster = Enemy('images/cyborg.png', x, y,40,40, 2,'left')
            ...
            if c=='d':
                ...
                items.add(door)
            if c=='o':
                coin = Player('images/coin.png', x, y,40,40, 0)
                coins.append(coin)
                items.add(coin)
            if c=='/':
                stair = Player('images/stair.png', x, y,100,240, 0)
                stairs.append(stair)
                items.add(stair)
            if c=='t':
                t = Player('images/cst_close.png', x, y,80,80, 0)
                items.add(t)
            x +=40
        y+=40
        x = 0
mana=Mana("images/mana.png",-150,0,20,20,3,'left')# предмет, що стріляє
